...
class t_sbt_busstop extends BaseModel
{
    ...
    {
        $this->logger->writeDebug("start##############");
        $this->logger->writeDebug($course_id ." ". $arrival ." ". $departure);
        $this->logger->writeDebug("end##############");
        // $op = $orientation > 0 ? '>' : '<';
//         $sql = <<< SQL
//             SELECT  DISTINCT
//                     BS.busstop_id,
...
SQL;
        ...
        // <!-- 路 ◯  出 なし 到 なし-->
        elseif ($course_id != 0 &&  $departure == 0 && $arrival == 0) {
            $sql = <<< SQL
                SELECT  DISTINCT
                        BS.busstop_id,
                        BSL.busstop_name,
                ...
                WHERE   1 = 1
                        ...
                        AND VDIA.course_id = :course_id
SQL;
            $param = array(":buscompany_id" => $buscompany_id, ":lang_cd" => 'ja',  ":course_id" => $course_id);
        }
        // <!-- 路 ◯  出 なし  到 ◯-->
        elseif ($course_id != 0 &&  $departure == 0 && $arrival != 0) {
            $sql = <<< SQL
...
SQL;
        ...
        elseif ($course_id == 0 && $departure == 0 && $orientation != 0) {
            $sql = <<< SQL
                SELECT  DISTINCT
                        SUBSTRING(BS.busstop_id, 1, CHAR_LENGTH(BS.busstop_id) - 2) busstop_id8,
                        BSL.busstop_name,
...
SQL;
        ...
        // <!-- 路 ◯  出 なし 到 なし  アクション 出 -->
        elseif ($course_id != 0 && $departure == 0 && $arrival == 0 && $orientation == 0) {
            $sql = <<< SQL
                SELECT  DISTINCT
                        SUBSTRING(BS.busstop_id, 1, CHAR_LENGTH(BS.busstop_id) - 2) busstop_id8,
                        BSL.busstop_name
